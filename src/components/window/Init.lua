...
return function(Config)
                ...
                for _, item in ipairs(liveData) do
                    ...
                    if title then
                        ...
                        if string.find(cleanExecName, cleanTitle) or string.find(cleanTitle, cleanExecName) then
                            ...
                            if logoUrl == "" then
                                -- Fall back to local logo if missing in API (e.g. Madium)
	...
	task.spawn(function()
		...
		conn = RunService.RenderStepped:Connect(function(dt)
			...
			if rightGradient1 then rightGradient1.Rotation = rot % 360 end
			if rightGradient2 then rightGradient2.Rotation = (360 - rot) % 360 end
		end)
	end)
